...
class SmartiRequests:
    ...
    def sendRequest(self, function, expectedResponse, *p):
        ...
        if (res.status_code != expectedResponse):
            self.logger.warning('Request '+str(function).split(' ')[2].split('.')[1]+' failed!\t\tErrorCode: '+str(res.status_code))
        else:
            self.logger.info('Request '+str(function).split(' ')[2].split('.')[1]+' successful')
    ...
    def cleanup(self, auth):
        ...
        cs = self.filterClientIds(self.sendRequest(self.getClient, SmartiRequests.GET_SUCCESS, auth).json())
